...
private val glfwKeyLocations =
  IntArray(GLFW_KEY_LAST + 1) { key -> glfwKeyLocationFromMapping(key) }

internal fun glfwKeyEvent(key: Int, scancode: Int, action: Int, mods: Int): KeyEvent? {
  val type =
    when (action) {
      GLFW_PRESS,
      GLFW_REPEAT -> KeyEventType.KeyDown
...
internal fun glfwKeyboardModifiers(
  mods: Int,
  scrollLockOn: Boolean = false,
...
private fun glfwKeyLocationFromMapping(key: Int): Int =
  when (key) {
    ...
    GLFW_KEY_LEFT_ALT,
    GLFW_KEY_LEFT_SUPER -> AwtKeyEvent.KEY_LOCATION_LEFT
    GLFW_KEY_RIGHT_SHIFT,
    GLFW_KEY_RIGHT_CONTROL,
    GLFW_KEY_RIGHT_ALT,
    GLFW_KEY_RIGHT_SUPER -> AwtKeyEvent.KEY_LOCATION_RIGHT
...
private fun glfwKeyCode(key: Int): Int = glfwKeyCodes.getOrElse(key) { AwtKeyEvent.VK_UNDEFINED }
...
private fun glfwKeyCodeFromMapping(key: Int): Int =
  when (key) {
    ...
    GLFW_KEY_LEFT_SUPER,
    GLFW_KEY_RIGHT_SUPER -> AwtKeyEvent.VK_META
